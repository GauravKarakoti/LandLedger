...
import { formatInr, shorten } from "@/lib/landledger";

// --- SERVER FUNCTIONS ---

const getDashboardData = createServerFn({ method: "GET" }).handler(async () => {
  // ✅ Dynamic imports ensure this code ONLY runs on the Node server
  const { db } = await import("@/server/db");
  const { disputes, events, transfers, properties } = await import("@/server/db/schema");
...
const getUserProperties = createServerFn({ method: "GET" })
  .validator((address: string) => address)
  .handler(async ({ data: ownerAddress }: any) => {
    // ✅ Dynamic imports here as well
    const { db } = await import("@/server/db");
...
function Dashboard() {
  ...
  // Fetch properties belonging to the connected wallet dynamically
  useEffect(() => {
  ...
  // Map database events to the format expected by the HistoryTimeline component
  const timelineEvents = recentEvents.map((e: any) => ({
    id: e.id,
    type: e.type,
    actor: "System", // Or extract from tx if available
